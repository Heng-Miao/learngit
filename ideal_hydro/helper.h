...
// parametrization of temperature dependent eta/s (for Pi)
incline real etaos(real temperature){
...
incline real minmod( real x, real y )
{
    real res = min( fabs(x), fabs(y) );
    return res*( sign(x) + sign(y) )*0.5f;
}

incline real4 minmod4( real4 x, real4 y)
{
...
incline real4 dudw( real4 ul, real4 ur, real dw )
{
    return ( ur - ul )/dw;
}
 ...
 * The maximum lam cam not be bigger than 1.0f, in relativity
 * if fluid velocity is v=1, maximum cs2=1/3,
 * the signal speed in computing frame is cs2' = (cs2+v)/(1+cs2*v) = 1.0 
 * */
incline real maxPropagationSpeed( real4 edv, real vk, read_only image2d_t eos_table)
{
...
/** solve energy density from T00 and K=sqrt(T01^2 + T02^2 + T03^)
 * 0.001% fail if absolute error < 0.01
 **/
incline void rootFinding(real* EdFind, real T00, real M, read_only image2d_t eos_table)
{
    real E0, E1;
    E1 = T00;   /*predict */
    real K2 = M*M;
...
incline void rootFinding_newton(real* ed_find, real T00, real M, 
    ...
    int i = 0;
    while (true){
        if ((f + df*(vh - vl))*(f + df*(vl - v)) > 0.0f || fabs(2.0f*f) > fabs(dvold*df)){
            dvold = dv;
            dv = 0.5f*(vh - vl);
            v = vl + dv;
        }
...
incline real4 t0m(real4 edv, real pr){
...
real4 kt1d(real4 ev_im2, real4 ev_im1, real4 ev_i, real4 ev_ip1,real4 ev_ip2,
    ...
    DA1 = DA0;  // reuse the previous calculation value
