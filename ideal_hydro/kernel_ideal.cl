...
__kernel void kt_src_christoffel(__global real4 * d_Src, __global real4*d_ev,
    ...
    if (I < NX*NY*NZ ) {
        ...
        // Tzz_tilde = T^{eta eta} * tau^2;
        real Tzz_tilde = (ed + pressure)*u0*u0*vz*vz + pressure;
        real Ttz_tilde = (ed + pressure)*u0*u0vz;


        d_Src = d_Src - (real4)(Tzz_tilde, 0.0f, 0.0f, Ttz_tilde);
    }

}

// output: d_Src; all the others are input
__kernel void kt_src_alongx(__global real4*dSrc,__global real4*d_ev,
                            read_only image2d_t eos_table,
                            const real tau){
...
// output: d_Src; all the others are input
__kernel void kt_src_alongy(__global real4*d_Src, __global real4*d_ev,
                            read_only image2d_t eps_table,
                            condt real tau){
    int I = get_global_id(0);
    int K = get_global_id(2);
    ...
    for ( int J = get_global_id(1); J < NY; J = J +BSZ ){
        ...
        d_Src[IND] = d_Src[IND] - tau*kt1d(ev[j-2], ev[j-1], ev[j], 
                    ev[j+1], ev[j+2], tau, ALONG_Y, eos_table)/DY;
    }
}

// output: d_Src; all the others are input
__kernel void kt_src_alongz(__global real4*d_Src, __global real4*d_ev,
                            real_only image2d_t eos_table, 
                            const real tau){
    int I = get_global_id(0);
    int J = get_global_id(1);
    ...
    barried(CLK_LOCAL_MEM_FENCE);

    for ( int K = get_global_id(2); K < NZ; K = K +BSZ ){
        int IND = I*NY*NZ + J*NZ + K;
        int k = K+2;

        d_Src[IND] = d_Src[IND] - tau*kt1d(ev[k-2], ev[k-1], ev[k],
                    ev[k+1], ev[k+2], tau, ALONG_Z, eos_table)/(tau*DZ);
    }
}

/** update d_evnew with d_ev1 and d_Src*/
__kernel void update_ev(__global real4*d_evnew, __global real4*d_ev1, 
                        __global real4*d_Src, read_only image2d_t eos_table,
                        const real tau, const int step){
    int I = get_global_id(0);
    if ( I < NX*NY*NZ ){
        real4 e_v + d_ev1[I];
        real ed = e_v.s0;
